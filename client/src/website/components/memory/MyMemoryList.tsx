...
class MyMemoryList extends React.Component<Props, States> {
    ...
    private renderMemories = (): React.ReactElement<any> => {
        if (this.props.state.memoryState.loading) {
            ...
            return <Fragment>
            {
                this.props.state.memoryState.myData
    ...
    private addFabActions = (): void => {
        if (this.props.state.userState.currentUser) {
            const editUri: string = "/mymemory/create";
    ...
    private renderCreateMemorySection = (): React.ReactElement<any> | undefined => {
        const memories: Memory [] = this.props.state.memoryState.myData;
        if (this.props.state.memoryState.loading) {
            return <Loading />;
            ...
            if (!memories || memories.length === 0) {
                return <Segment placeholder>
                    ...
                    <FormattedMessage id="page.memory.my_empty" />
                    </Header>
                </Segment>;
            }
        } else {
    ...
    private loadMore = (createdAt: string): void => {
        this.props.actions.getMoreMyMemories(createdAt);
    }
}

export default connectAllProps(MyMemoryList);